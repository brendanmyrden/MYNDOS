...
type RoundedBoxGeometryModule = typeof import("three/examples/jsm/geometries/RoundedBoxGeometry.js");
type MeshPhysicalMaterialParameters = import("three").MeshPhysicalMaterialParameters;
...
export default function PlusCube({ moduleName }: PlusCubeProps) {
  ...
  useEffect(() => {
    ...
    const setupRenderer = async () => {
      ...
      let RoundedBoxGeometry: RoundedBoxGeometryModule["RoundedBoxGeometry"];

      try {
        const [threeModule, roundedBoxModule] = await Promise.all([
          import("three"),
        ...
        if (!cancelled) {
          console.warn("PlusCube WebGL dependencies unavailable; rendering static fallback.", error);
          showFallback();
        }
        return;
      }
      ...
      const sharedGlassProps = {
        ...
        transparent: true,
        opacity: 0.72,
      } satisfies Partial<MeshPhysicalMaterialParameters>;
